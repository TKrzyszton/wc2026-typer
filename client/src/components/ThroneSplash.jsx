...
export default function ThroneSplash() {
  ...
  useEffect(() => {
    if (sessionStorage.getItem('throne_splash_seen')) return;
    sessionStorage.setItem('throne_splash_seen', '1');
    setVisible(true);
  ...
  return (
    ...
    >
      <style>{`
        ...
        @keyframes bow { 0%,100% { transform: rotate(0deg); } 40%,70% { transform: rotate(-55deg); } }
        ...
        .ts-bow1 { animation: bow 2.6s ease-in-out infinite; transform-origin: 205px 168px; }
        .ts-bow2 { animation: bow 2.6s ease-in-out 0.3s infinite; transform-origin: 253px 168px; }
        .ts-bow3 { animation: bow 2.6s ease-in-out 0.6s infinite; transform-origin: 301px 168px; }
      ...
      <div className="flex flex-col items-center gap-4 select-none px-4">
        <svg viewBox="0 0 340 210" className="w-full max-w-md" xmlns="http://www.w3.org/2000/svg">
          ...
          {[
            ...
            <g key={x}>
              ...
              <g stroke="#a8a29e" strokeWidth="3" strokeLinecap="round" fill="none">
                <path d={`M${x} 168 L${x - 8} 190`} />
                <path d={`M${x} 168 L${x + 8} 190`} />
              </g>
              {/* górna połowa kłania się */}
              <g className={cls}>
                <g stroke="#a8a29e" strokeWidth="3" strokeLinecap="round" fill="none">
                  <path d={`M${x} 168 L${x} 140`} />
                  <path d={`M${x} 148 L${x - 12} 158`} />
                  <path d={`M${x} 148 L${x + 6} 160`} />
                </g>
                <circle cx={x} cy="132" r="8" fill="#0a1628" stroke="#a8a29e" strokeWidth="3" />
              </g>
            </g>
          ))}
          ...
          <text x="253" y="205" textAnchor="middle" fill="#78716c" fontSize="10" fontFamily="sans-serif">plebs</text>
        </svg>
